...
#place random food on screen
    while not foodmade:
      ...
      if screen.inch(y, x) == ord(' '):
        foodmade = True
        screen.addch(y, x, ord('$') | curses.color_pair(3) | curses.A_BOLD)
...
#movement
    ...
    screen.addch(head[0], head[1], ord('#'), curses.color_pair(6) | curses.A_BOLD) 
    action = screen.getch()
...
#Gameover Menu 
  ...
  while selection < 0:
    ...
    screen.addstr(9, dims[1]/2-4, 'GAMEOVER', curses.color_pair(1) | curses.A_BOLD)
   # screen.addstr(dims[0]/2+1, dims[1]/2-11, 'Please Select:', curses.color_pair(4))
    screen.addstr(dims[0]/2-1, dims[1]/2-9, 'You got '+str(len(body)-5)+ ' points!', curses.color_pair(5))
    screen.addstr(dims[0]/2+2, dims[1]/2-4, 'New Game', graphics[0])
    screen.addstr(dims[0]/2+4, dims[1]/2-7, 'Return to Menu', graphics[1])
    screen.addstr(dims[0]/2+6, dims[1]/2-2, 'Exit', graphics[2])
    screen.refresh()
    action = screen.getch()
...
def menu():
  ...
  option = 0
  while selection < 0:
    screen.addstr(dims[0]-1, dims[1]-6, 'v0.12', curses.A_BOLD)
    graphics = [0]*4
  ...
  |__)~~\__|__|__|  \.__/| \/~~\  \___ 
                                       ''', curses.color_pair(3) | curses.A_BOLD | curses.A_REVERSE)
    screen.addstr(dims[0]/2-2, dims[1]/2-4, 'Play!', graphics[0])
    screen.addstr(dims[0]/2, dims[1]/2-5, 'Options', graphics[1])
    screen.addstr(dims[0]/2+2, dims[1]/2-5, 'Credits', graphics[2])
    screen.addstr(dims[0]/2+4, dims[1]/2-4, 'Exit?', graphics[3])
    screen.refresh()
    action = screen.getch()
    if action == curses.KEY_UP:
      option = (option - 1) % 4
...
def options():
  ...
  while selection < 0:
    ...
    screen.addstr(2, dims[1]/2-len('Options')+1, 'OPTIONS', curses.color_pair(1) | curses.A_BOLD)
    screen.addstr(dims[0]/3, dims[1]/4, 'Colors:', curses.A_BOLD)
...
def credits():
  ...
  while action == 0:
    screen.addstr(dims[0]-1, dims[1]-6, 'v0.12', curses.A_BOLD)
    screen.addstr(dims[0]/2-2, dims[1]/2-17, 'Bert made dis shiz! watch yoself!')
    screen.refresh()
    action = screen.getch()
    if action == ord('\n'):
      menu()    
